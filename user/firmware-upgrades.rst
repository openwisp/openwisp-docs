...
on allowing users to maintain the firmware of their network devices up to
date.
...
See `Enabling the firmware upgrader module on the OpenWISP 22.05 ansible
role documentation
...
A ``DeviceFirmware`` object represent the relationship between a device
and a firmware image, it basically tells us which firmware image is
...
time. Alternatively the `REST API
<https://github.com/openwisp/openwisp-firmware-upgrader/tree/1.0#rest-api>`__
...
pull-request to add it, otherwise you can use `the setting
...
For more information about the features offered by OpenWISP Firmware
Upgrader we refer to the following sections of its documentation:
